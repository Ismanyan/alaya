...
class Absent_model extends CI_Model {
    ...
    // Request absent user
    public function getAbsent(INT $id, $date=null)
    {
        $url = getenv('REST_URL');
        $token = $this->session->userdata('TOKEN');
        if ($date == null) {
            $response = Requests::GET($url . 'absent/get/' . $id . '?token=' . $token);
        } else {
            $response = Requests::GET($url . 'absent/get/'. $id.'?date=' . $date . '&token=' . $token);
        }
